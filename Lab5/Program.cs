using System;
using System.Globalization;
using System.Linq;
using System.Net.Security;
using System.Text;

namespace Lab5
{
    class Program
    {
        static int[] initialPermutationTable = new int[]
        {
            58,50,42,34,26,18,10,2,60,52,44,36,28,20,12,4,
            62,54,46,38,30,22,14,6,64,56,48,40,32,24,16,8,
            57,49,41,33,25,17,9,1,59,51,43,35,27,19,11,3,
            61,53,45,37,29,21,13,5,63,55,47,39,31,23,15,7
        };
        static int[] inverseInitialPermutationTable1 = new int[]
        {
            40, 8, 48, 16, 56, 24, 64,
            32, 39, 7, 47, 15, 55,
            23, 63, 31, 38, 6, 46,
            14, 54, 22, 62, 30, 37,
            5, 45, 13, 53, 21, 61,
            29, 36, 4, 44, 12, 52,
            20, 60, 28, 35, 3, 43,
            11, 51, 19, 59, 27, 34,
            2, 42, 10, 50, 18, 58,
            26, 33, 1, 41, 9, 49,
            17, 57, 25
        };
        static int[] firstKeyHePermutationTable = { 57, 49, 41, 33, 25,
                      17, 9, 1, 58, 50, 42, 34, 26,
                      18, 10, 2, 59, 51, 43, 35, 27,
                      19, 11, 3, 60, 52, 44, 36, 63,
                      55, 47, 39, 31, 23, 15, 7, 62,
                      54, 46, 38, 30, 22, 14, 6, 61,
                      53, 45, 37, 29, 21, 13, 5, 28,
                      20, 12, 4 };
        static int[] secondKeyCompressionPermutationKey = new int[]
        {
            14,17,11,24,1,5,3,28,15,6,21,10,
            23,19,12,4,26,8,16,7,27,20,13,2,
            41,52,31,37,47,55,30,40,51,45,33,48,
            44,49,39,56,34,53,46,42,50,36,29,32
        };
        static int[] ExpansionDBoxP = { 32, 1, 2, 3, 4, 5, 4,
                     5, 6, 7, 8, 9, 8, 9, 10,
                     11, 12, 13, 12, 13, 14, 15,
                     16, 17, 16, 17, 18, 19, 20,
                     21, 20, 21, 22, 23, 24, 25,
                     24, 25, 26, 27, 28, 29, 28,
                     29, 30, 31, 32, 1 };
        // Straight Permutation Table 
        static int[] P = { 16, 7, 20, 21, 29, 12, 28,
                    17, 1, 15, 23, 26, 5, 18,
                    31, 10, 2, 8, 24, 14, 32,
                    27, 3, 9, 19, 13, 30, 6,
                    22, 11, 4, 25 };
        //SBoxTable
        static int[][][] sbox = {
            new[]{
              new[]{ 14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7 },
              new[] { 0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8 },
              new[] { 4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0 },
              new[] { 15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13 }
            },

            new[]{
              new []{ 15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10 },
              new []{ 3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5 },
              new []{ 0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15 },
              new []{ 13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9 }
            },
            new[]{
              new[] { 10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8 },
              new[] { 13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1 },
              new[] { 13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7 },
              new[] { 1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12 }
            },
            new[] {
                new[]{ 7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15 },
                new[]   { 13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9 },
                new[]   { 10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4 },
                new[]   { 3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14 }
            },
            new[] {
                new[]{ 2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9 },
                new[]   { 14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6 },
                new[]   { 4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14 },
                new[]   { 11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3 }
            },
            new[] {
                new[]{ 12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11 },
                new[]   { 10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8 },
                new[]   { 9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6 },
                new[]   { 4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13 }
            },
            new[] {
                new[]{ 4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1 },
                new[]   { 13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6 },
                new[]   { 1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2 },
                new[]   { 6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12 }
            },
            new[] {
                new[]{ 13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7 },
                new[]   { 1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2 },
                new[]   { 7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8 },
                new[]   { 2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11 }
            }
        };
        static int[] shiftBits = { 1, 1, 2, 2, 2, 2, 2, 2,
                            1, 2, 2, 2, 2, 2, 2, 1 };
        static void Main(string[] args)
        {
            string text = "123456ABCD132536";
            string key = "AABB09182736CCDD";
            text = Encrypt(text, key);
            Console.WriteLine("\nCipher Text: " + text.ToUpper() + "\n");

        }
        public static string Encrypt(string plainText, string key)
        {
            int i;
            // get round keys 
            string[] keys = getKeys(key);

            // initial permutation 
            plainText = Permutation(initialPermutationTable, plainText);
            Console.WriteLine(
                "After initial permutation: "
                + plainText.ToUpper());
            Console.WriteLine(
                "After splitting: L0="
                + plainText.Substring(0, 8).ToUpper()
                + " R0="
                + plainText.Substring(8, 8).ToUpper() + "\n");

            // 16 rounds 
            for (i = 0; i < 16; i++)
            {
                plainText = Round(plainText, keys[i], i);
            }

            // 32-bit swap 
            plainText = plainText.Substring(8, 8)
                        + plainText.Substring(0, 8);

            // final permutation 
            plainText = Permutation(inverseInitialPermutationTable1, plainText);
            return plainText;
        }


        static string Round(string input, string key, int num)
        {
            // fk 
            string left = input.Substring(0, 8);
            string temp = input.Substring(8, 8);
            string right = temp;
            // Expansion permutation 
            temp = Permutation(ExpansionDBoxP, temp);
            // xor temp and round key 
            temp = Xor(temp, key);
            // lookup in s-box table 
            temp = SBox(temp);
            // Straight D-box 
            temp = Permutation(P, temp);
            // xor 
            left = Xor(left, temp);
            Console.WriteLine("Round "
                               + (num + 1) + " "
                               + right.ToUpper()
                               + " " + left.ToUpper() + " "
                               + key.ToUpper());

            // swapper 
            return right + left;
        }

        // xor 2 hexadecimal strings 
        static string Xor(string a, string b)
        {
            // hexadecimal to decimal(base 10) 
            //long t_a = Long.parseUnsignedLong(a, 16);   //**************************************************************************
            // hexadecimal to decimal(base 10) 
            //long t_b = Long.parseUnsignedLong(b, 16);   //**************************************************************************

            long t_a = Convert.ToInt64(a, 16);
            long t_b = Convert.ToInt64(b, 16);


            // xor 
            t_a = t_a ^ t_b;
            // decimal to hexadecimal 
            a = t_a.ToString("X");
            //a = Convert.ToString(t_a,16); //Long.toHexString(t_a)
            // prepend 0's to maintain length 
            while (a.Length < b.Length)
                a = "0" + a;
            return a;
        }

        // s-box lookup 
        static string SBox(string input)
        {
            string output = "";
            input = HextoBin(input);
            for (int i = 0; i < 48; i += 6)
            {
                string temp = input.Substring(i, 6);
                int num = i / 6;
                int row = Convert.ToInt32(
                    temp[0] + "" + temp[5], 2);//charAt();
                int col = Convert.ToInt32(
                    temp.Substring(1, 4), 2);
                output += Convert.ToString(
                    sbox[num][row][col], 16);//konwersja do 16-tkowego hexa
            }
            return output;
        }






        static string[] getKeys(string key)
        {
            string[] keys = new string[16];
            // pierwszy klucz permutacji
            key = Permutation(firstKeyHePermutationTable, key);
            for (int i = 0; i < 16; i++)
            {
                key = LeftCircularShift(
                          key.Substring(0, 7), shiftBits[i])
                      + LeftCircularShift(key.Substring(7, 7), //substring??????
                                          shiftBits[i]);
                // second key permutation 
                keys[i] = Permutation(secondKeyCompressionPermutationKey, key);
            }
            return keys;
        }
        // left Circular Shifting bits 
        static string LeftCircularShift(string input, int numBits)
        {
            int n = input.Length * 4;
            int[] perm = new int[n];
            for (int i = 0; i < n - 1; i++)
                perm[i] = (i + 2);
            perm[n - 1] = 1;
            while (numBits-- > 0)
                input = Permutation(perm, input);
            return input;
        }
        static string Permutation(int[] sequence, string input)
        {
            string output = "";
            input = HextoBin(input);
            for (int i = 0; i < sequence.Length; i++)
                output += input[sequence[i] - 1];
            output = BinToHex(output);
            return output;
        }
        // binary to hexadecimal conversion 
        static string BinToHex(string input)
        {
            int n = (int)input.Length / 4;

            //input = Long.toHexString( 
            //Long.parseUnsignedLong(input, 2));
            input = Convert.ToInt64(input, 2).ToString("X"); //******************TU COŚ MOŻE BYĆ ŹLE************************************
            //input = Convert.ToString(
            //    Long.parseUnsignedLong(input, 2),16);//Long.toHexString//**************************************************************************
            while (input.Length < n)
                input = "0" + input;
            return input;
        }
        // hexadecimal to binary conversion 
        static string HextoBin(string input)
        {
            int n = input.Length * 4;
            string binarystring = String.Join(String.Empty,
                  input.Select(
                    c => Convert.ToString(Convert.ToInt32(c.ToString(), 16), 2).PadLeft(4, '0')
                  )
                );
            //long l = long.Parse(input,NumberStyles.HexNumber);
            //input = l.ToString("D16");
            //input = Convert.ToUInt64(long.Parse(input, NumberStyles.HexNumber).ToString("D16"), 2).ToString("X");

            //StringBuilder sb = new StringBuilder();
            //var bytes = Encoding.Unicode.GetBytes(input);
            //foreach (var t in bytes)
            //{
            //    sb.Append(t.ToString("X2"));
            //}
            ///*dousuiniecia
            ////foreach (char c in input.ToCharArray())
            ////{
            ////    sb.Append(Convert.ToString(c, 2).PadLeft(8, '0'));
            ////}
            //*/
            //input = sb.ToString();
            ////            input = Long.toBinaryString(
            ////                Long.parseUnsignedLong(input, 16));

            ////input = Convert.ToString(
            ////   hexString,2);//Long.toBinaryString //**************************************************************************
            ////input = Convert.ToString(Convert.ToInt64(input, 16), 2);
            //input=String.Join(String.Empty,
            //      input.Select(
            //        c => Convert.ToString(Convert.ToInt32(c.ToString(), 16), 2).PadLeft(4, '0')
            //      )
            //    );
            //while (input.Length < n)
            //    input = "0" + input;
            return binarystring;
        }
    }
}
